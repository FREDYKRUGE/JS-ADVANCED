...
const detailsTemp = (pet, isOwner,hasUser, onDelete) => html`
...
export async function showDetails(ctx) {
    ...
    ctx.render(detailsTemp(pet, isOwner, hasUser, onDelete))

    async function onDelete() {
        const choice = confirm('Are u sure u want to del this pet');
        if (choice) {
            await deleteById(id);
            ctx.page.redirect('/')
        }
